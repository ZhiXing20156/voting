...
from evm import models


class OpLogs(MiddlewareMixin):

    __exclude_urls = ['index/','download/','create/']  # 定义不需要记录日志的url名单
    ...
    def process_response(self, request, response):
        ...
        if self.data.get('access_time') > 3 * 1000:
            ...
            models.AccessTimeOutLogs.objects.create(**self.data)  # 超时操作日志入库db
        models.OpLogs.objects.create(**self.data)  # 操作日志入库db
